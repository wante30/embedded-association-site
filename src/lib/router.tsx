...
import { stripBasePath, withBasePath } from './basePath.mjs';
...
const snapshot = () => `${stripBasePath(window.location.pathname, baseUrl)}${window.location.search}`;
...
export function Link({ to, onClick, children, ...props }: AnchorHTMLAttributes<HTMLAnchorElement> & { to: string }) {
  const href = withBasePath(to, baseUrl);
